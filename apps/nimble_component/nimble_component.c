...
char characteristic_value[50] = "Hello, are you trying to read me?";  // When client read characteristic, he get this value. You can also set this value in your code.
...
void handle_write_command(char *command) {
  printf("Received: %s\n", command); // Print the received value

  if (strcmp(command, "stop") == 0) {
    stopBLE();
  } else if (strcmp(command, "toggle_led") == 0) {
    printf("running toggleLED() function.");
  }
}
...
void sendNotification() {
  ...
  if (notify_state) {
    ...
    rc = ble_gatts_notify_custom(conn_handle, notification_handle, om); // ! ble_gattc_notify_custom -> ble_gatts_notify_custom
...
void startBLE() {
  ...
  ESP_ERROR_CHECK(ret);
  nimble_port_init();
  // ESP_ERROR_CHECK(esp_nimble_hci_and_controller_init()); // ! removed in latest version

  /* Initialize the NimBLE host configuration. */
  ble_hs_cfg.reset_cb = bleprph_on_reset; // on restart
  ...
  rc = ble_svc_gap_device_name_set("Nahhas Device"); //  Set the name of this device
  assert(rc == 0);
...
// Call this function to stop BLE
void stopBLE() {
  ...
  if (ret == 0) {
    nimble_port_deinit();
    // ret = esp_nimble_hci_and_controller_deinit(); // ! removed in latest verison
...
// Mandatory to initialize NVS at the start.
void startNVS() {
